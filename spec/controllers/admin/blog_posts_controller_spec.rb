...
RSpec.describe Admin::BlogPostsController, type: :controller do
  ...
  describe 'GET index' do
    ...
    it 'while has access' do
      setup
      stub_const('User::CAN_ACCESS_BLOG_POSTS', 'true')

      get :index
      expect(User::CAN_ACCESS_BLOG_POSTS).to eq('true')
      expect(response).to be_success
    end

    it 'while has no access' do
      setup
      stub_const('User::CAN_ACCESS_BLOG_POSTS', 'false')

      get :index
      expect(User::CAN_ACCESS_BLOG_POSTS).to eq('false')
